...
##############################################################################
    ...
    with open(lastz_output_file, "w") as f:
        f.write(lastz_output)

    print(f"LastZ alignment saved to: {lastz_output_file}")
...
##############################################################################  
    ...
    # Split alignments into chunks for parallel processing
    chunk_size = max(1, len(maf_alignments) // NUM_PROCESSES)
    alignment_chunks = list(chunks(maf_alignments, chunk_size))

    # Prepare data for parallel processing
    chunk_data = [(chunk, genes, restriction_site) for chunk in alignment_chunks]
    
    # Process chunks in parallel
    segments = []
    with concurrent.futures.ProcessPoolExecutor(max_workers=NUM_PROCESSES) as executor:
        futures = [executor.submit(process_alignment_chunk, data) for data in chunk_data]
    ...
    print(f"Identified {len(segments)} potential sequence segments.")
