...
class PrimerDesignPipeline:
    ...
    def run_lastz(self, maf_choice):
        """Run LastZ alignments if needed."""
        if maf_choice == "1":
            lastz_output_files = []
            ...
            for ref_genome_path, qry_genome_path in combinations(genome_paths, 2):
                alignment_name = (f"{os.path.splitext(os.path.basename(ref_genome_path))[0]}_"
                                 f"vs_{os.path.splitext(os.path.basename(qry_genome_path))[0]}_"
                                 f"{time.strftime('%Y%m%d-%H%M%S')}.maf")
                lastz_output_file = os.path.join(self.script_dir, f"lastz_alignment_{alignment_name}")
    ...
    def run_multiz(self):
        ...
        if self.use_multiz and len(self.maf_files) > 1:
            multiz_output_file = os.path.join(self.script_dir, f"multiz_alignment_{time.strftime('%Y%m%d-%H%M%S')}.maf")
    ...
    def save_results(self, df):
        ...
        output_file = os.path.join(self.script_dir, f"Primer_Results_{timestamp}.xlsx")
