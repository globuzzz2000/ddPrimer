...
# Primer3 and MultiZ paths (moved from inline definitions)
PRIMER3_CORE_PATH = "/opt/homebrew/bin/primer3_core"
MULTIZ_PATH = "/Library/Application Support/Multi Sequence Alignment/multiz"
    ...
    with open(maf_path, 'r') as maf_file:
        for line in maf_file:
            ...
            if len(chunk_lines) >= chunk_size:
                ...
                segments.extend(chunk_processor(alignments))
...
def main():
    ...
    num_genomes = 2 if not use_multiz else int(input("How many genomes? (≥3): ").strip())
    ...
    if maf_choice == "1":
        ...
        maf_files = filedialog.askopenfilenames(title="Select MAF files", filetypes=[("MAF Files", "*.maf"), ("All Files", "*.2*")])
        if not maf_files:
            print("No MAF files selected. Exiting.")
            sys.exit(1)
    ...
    if num_extra > 0:
        ...
        for i in range(num_extra):
            extra_path = get_file(f"Select additional genome {i+1} FASTA File", [("FASTA Files", "*.fasta *.fa *.fna"), ("All Files", "*.*")])
    ...
    if maf_choice == "1":
        ...
        genome_paths = [ref_path, qry_path] + extra_genomes

        # Run LastZ pairwise for all genome combinations
        for i in range(len(genome_paths) - 1):
            for j in range(i + 1, len(genome_paths)):
                ref_genome_path = genome_paths[i]
                qry_genome_path = genome_paths[j]

                lastz_output_file = os.path.join(script_dir, f"lastz_alignment_{i}_{j}_{time.strftime('%Y%m%d-%H%M%S')}.maf")
                lastz_output_files.append(lastz_output_file)

                lastz_cmd = [
                    "lastz", f"{ref_genome_path}[multiple]", f"{qry_genome_path}[multiple]",
                    "--format=maf", "--ambiguous=iupac",
                    "--output=" + lastz_output_file
                ]
                
                print(f"\nRunning LastZ for {ref_genome_path} vs {qry_genome_path}... ")
    ...
    if use_multiz:
        multiz_output_file = os.path.join(script_dir, f"multiz_alignment_{time.strftime('%Y%m%d-%H%M%S')}.maf")
        multiz_cmd = [
            MULTIZ_PATH,
            *maf_files, "R=30", "M=1", multiz_output_file
        ]
 
        print("Running MultiZ for multiple sequence alignment... ", end="", flush=True)
        stop_event = threading.Event()
        thread = threading.Thread(target=spinner_task, args=(stop_event,))
        thread.start()
 
        try:
            proc = subprocess.Popen(multiz_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
            multiz_output, multiz_error = proc.communicate()
        except Exception as e:
            print(f"\nError executing MultiZ: {e}")
            sys.exit(1)
        ...
        print(f"\nMultiZ alignment saved to: {multiz_output_file}")
...
##############################################################################
    ...
    print("Extracting sequences from MAF alignments...")
    segments = []

    for maf_file in maf_files:
        segments.extend(list(process_maf_file_in_chunks(maf_file, lambda alns: list(collect_segments_from_alignments(alns, genes)), MAF_CHUNK_SIZE)))
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=MEMORY_INTENSIVE_PROCESSES) as executor:
        ...
        validate_batch = functools.partial(validate_sequences_batch, ref_genome, qry_genome, extra_genomes)
