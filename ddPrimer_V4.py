...
# Tool paths
PARALLEL_LASTZ_PATH = "/Library/Application Support/ddPrimer/Parallel LastZ/parallelLastz.pl"
PARALLEL_LASTZ_CONFIG = "/Library/Application Support/ddPrimer/Parallel LastZ/parallelLastz.config"
...
def get_file(prompt, filetypes):
    ...
    if not file_path:
        print(f"[ERROR] {prompt} - No file selected. Exiting.")
        sys.exit(1)
...
def get_files(prompt, filetypes):
    ...
    if not file_paths:
        print(f"[ERROR] {prompt} - No files selected. Exiting.")
        sys.exit(1)
...
def main():
    ...
    if maf_choice == "1":
        ...
        for ref_genome_path, qry_genome_path in combinations(genome_paths, 2):
            ...
            # Modify the parallelLastZ command to place output directly in the alignments directory
            lastz_cmd = [
                "perl", PARALLEL_LASTZ_PATH,
                "--qfile", qry_genome_path,
                "--tfile", ref_genome_path,
                "--cfile", PARALLEL_LASTZ_CONFIG,
                "--speedup", str(NUM_PROCESSES),
                "--length", "1000",
                "--output", alignments_dir  # Output directly to the alignments directory
            ]
            ...
            # Find the MAF files that were just created in the alignments directory
            # They should have a specific naming pattern from parallelLastZ: "seeALN_*.maf"
            alignment_pattern = f"seeALN_*.maf"
            generated_maf_files = glob.glob(os.path.join(alignments_dir, alignment_pattern))

            if generated_maf_files:
                print(f"[INFO] Found {len(generated_maf_files)} LastZ alignment files in {alignments_dir}")
                # Add these files to our tracking list
                lastz_output_files.extend(generated_maf_files)
            else:
                print(f"[WARNING] No MAF files found in alignments directory matching pattern: {alignment_pattern}")

            # No need for temp directory cleanup as we didn't create one
