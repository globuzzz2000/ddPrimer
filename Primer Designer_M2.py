...
def main():
    ...
    if maf_choice == "1":
        ...
        for i in range(len(genome_paths) - 1):
            for j in range(i + 1, len(genome_paths)):
                ...
                print(f"\nRunning LastZ for {os.path.basename(ref_genome_path)} vs {os.path.basename(qry_genome_path)}... ")
    ...
    if use_multiz and len(maf_files) > 1:  # Only run MultiZ if we have multiple MAF files
        ...
        # Prepare MultiZ command
        multiz_cmd = [MULTIZ_PATH]
        multiz_cmd.extend(maf_files)
        multiz_cmd.extend(["R=30", "M=1", multiz_output_file])
    ...
    print(f"Extracted {len(segments)} initial segments from MAF files.")
    ...
    for i in range(len(df)):
        ...
        keepP = pd.isna(row["Probe"]) or row["Probe"] == "" or passes_blast_filter(row, "Probe")  # Only check probes if they exist
