...
def main():
    ...
    if maf_choice == "1":
        ...
        # Run LastZ pairwise for all genome combinations using all possible pairs
        from itertools import combinations
        for ref_genome_path, qry_genome_path in combinations(genome_paths, 2):

            alignment_name = f"{os.path.splitext(os.path.basename(ref_genome_path))[0]}_vs_{os.path.splitext(os.path.basename(qry_genome_path))[0]}_{time.strftime('%Y%m%d-%H%M%S')}.maf"
            lastz_output_file = os.path.join(script_dir, f"lastz_alignment_{alignment_name}")
